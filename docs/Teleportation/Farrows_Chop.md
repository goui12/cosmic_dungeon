...
- The departure escrows the dungeon inventory and restores the player's outside inventory. The outside inventory is saved separately while visiting Main Village and the dungeon inventory is restored only on a valid return.
- Currency deposits and withdrawals are disabled during this temporary outside visit, preventing currency from crossing the dungeon inventory boundary.
...
If the lifecycle ends while its owner is outside, cleanup keeps the outside inventory and discards the isolated dungeon inventory according to the normal run boundary.
