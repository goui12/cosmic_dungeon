...
- Farrow's Chop temporarily separates dungeon and outside inventories, persists both sides across restart, and blocks currency transactions during the outside visit. Template-authored D1 achievement and Plant Flags regions now resolve correctly inside physical instances.
- Existing dungeon-run SavedData gains an optional slot field with startup migration for active legacy records. A new additive inventory-escrow SavedData file has no legacy migration requirement. Rift destination, region, door/key, Cosmic Spawner block-entity/preset, currency, faction, progression, class, and achievement schemas remain unchanged. Back up the full world before upgrading; no placed spawner recreation is required.
