...
- Own offer slots can only be edited while the player's offer is not ready/locked; item or Trace currency changes reset both players' ready/confirm state, and shift-click routes only between the player's inventory/hotbar and their own offer slots.
- Added trade network payloads via ModNetwork for request/accept/currency/ready/confirm/cancel, the CAPS LOCK look-target trade request payload, and server-to-client `S2C_TradeState` sync. Trade state is view-specific per player and keeps the screen updated with self/other names, Trace balances, Trace offers, ready/confirm status, and status messages while item offers continue syncing through menu slots; typed `/trade <player>` remains the distance-free fallback, while the hotkey is server-validated for same-dimension online players within close range.
