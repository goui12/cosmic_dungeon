...
- The authorized bottom-right Cosmic Spawner preview is now dynamically assembled from enabled fields only, with no gaps for disabled rows. Its height follows actual rendered rows, its width follows content up to `spawnerHud.maxWidth`, and long text wraps within that cap.
