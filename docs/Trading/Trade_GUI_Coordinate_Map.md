...
- TradeMenu slot constants:
  ...
  - the default vanilla title/inventory labels and vanilla inventory background are suppressed; the PNG background is the GUI. Custom text draws the synced partner/self names and compact ready/finalize status after the real container slots/items render, including waiting/finalize prompts for the two-phase active trade state.
