...
- Cameron authorized the development-client handoff workflow on 2026-09-24: after each completed edit pass, run the applicable datagen task(s), review generated changes, then run the Gradle build with Java 21. When asking Cameron to test, launch runClient and provide a short, change-specific test list. This standing authorization supersedes the earlier requirement to ask before each development-client launch. See the workflow section below.
- Local dedicated-server/GameTest launches and installed-client/server deployment still require their separate authorization. Development-client checks do not replace realistic gameplay QA as **Goui12** using his legitimate Microsoft login and the licensed TEST NeoForge server.
...
### Datagen, Build and Client Handoff (Cameron update, 2026-09-24)
...
- After successful build and relevant checks, automatically launch runClient whenever asking Cameron to test the current work. Do not ask for the same launch permission again. Check whether a client is already running; do not force-close an active game/world or start duplicate clients. If the existing client cannot show the new build, explain that a restart is needed.
- Check startup with bounded logs/window status and fix failures caused by the current changes before handing the client over. Leave the client available for Cameron; do not enter worlds or perform destructive gameplay automatically.
