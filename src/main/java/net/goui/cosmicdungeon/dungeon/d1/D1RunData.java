...
public final class D1RunData extends SavedData {
    ...
    private D1RunData() {}
    public static D1RunData get(MinecraftServer server) { return server.overworld().getDataStorage().computeIfAbsent(TYPE); }
    private static D1RunData load(List<State> states, Map<String, List<String>> credits) {
        ...
        for (State s : states) if (s.runId() > 0) data.runs.put(s.runId(), s);
        return data;
    }
