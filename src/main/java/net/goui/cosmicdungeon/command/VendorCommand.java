...
public final class VendorCommand {
    ...
    private static int usage(CommandSourceStack src) { src.sendFailure(Component.literal("Usage: /vendor list|reload|profile <profileId>|assign <profileId>|clear|info|spawn <profileId>")); return 0; }
    ...
    private static int profile(CommandSourceStack src, String profileIdRaw) {
        ...
        src.sendSuccess(() -> Component.literal(" type=" + profile.vendorType() + ", requiredFaction=" + profile.requiredFactionId() + ", requiredFactionTier=" + profile.requiredFactionTier() + ", requiredFlag=" + profile.requiredProgressionFlag() + ", requiredNpcTier=" + profile.requiredNpcTier()), false);
