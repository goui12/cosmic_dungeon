...
public interface VertexSorting {
    ...
    static VertexSorting byDistance(VertexSorting.DistanceFunction distanceFunction) {
        return p_445123_ -> {
            ...
            float[] afloat = new float[p_445123_.size()];
            int[] aint = new int[p_445123_.size()];

            for (int i = 0; i < p_445123_.size(); aint[i] = i++) {
                afloat[i] = distanceFunction.apply(p_445123_.get(i, vector3f));
            }
    ...
    int[] sort(CompactVectorArray vectors);
