...
public record HashedPatchMap(Map<DataComponentType<?>, Integer> addedComponents, Set<DataComponentType<?>> removedComponents) {
    ...
    public static HashedPatchMap create(DataComponentPatch patch, HashedPatchMap.HashGenerator hashGenerator) {
        DataComponentPatch.SplitResult datacomponentpatch$splitresult = patch.split();
        Map<DataComponentType<?>, Integer> map = new IdentityHashMap<>(datacomponentpatch$splitresult.added().size());
        datacomponentpatch$splitresult.added().forEach(p_412727_ -> map.put(p_412727_.type(), hashGenerator.apply((TypedDataComponent<?>)p_412727_)));
