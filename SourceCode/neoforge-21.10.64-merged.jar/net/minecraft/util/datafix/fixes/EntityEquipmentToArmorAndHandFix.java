...
public class EntityEquipmentToArmorAndHandFix extends DataFix {
        ...
        if (!type.equals(this.getInputSchema().getType(References.ENTITY_EQUIPMENT))) {
            ...
            return TypeRewriteRule.seq(
                this.fixTypeEverywhereTyped(
                    ...
                    p_396575_ -> p_396575_.update(DSL.remainderFinder(), EntityEquipmentToArmorAndHandFix::fixDropChances)
                ),
                this.fixTypeEverywhere(
                    ...
                    type1,
                    p_396584_ -> {
                        ItemStackNew itemstacknew = newItemStackType.read(new Dynamic<>(p_396584_).emptyMap())
                            .result()
                        ...
                        return p_396581_ -> p_396581_.mapSecond(p_396578_ -> {
                            List<ItemStackOld> list = p_396578_.map(Function.identity(), p_396582_ -> List.of());
    ...
    private static Dynamic<?> fixDropChances(Dynamic<?> data) {
        ...
        if (optional.isPresent()) {
            Iterator<Float> iterator = Stream.concat(optional.get().map(p_396585_ -> p_396585_.asFloat(0.0F)), Stream.generate(() -> 0.0F)).iterator();
            ...
            if (data.get("ArmorDropChances").result().isEmpty()) {
                data = data.set(
                    "ArmorDropChances",
                    data.createList(Stream.of(iterator.next(), iterator.next(), iterator.next(), iterator.next()).map(data::createFloat))
                );
            }
        }
