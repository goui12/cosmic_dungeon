...
public class DebugSubscriptions<T> {
    ...
    private static DebugSubscription<?> registerSimple(String name) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, ResourceLocation.withDefaultNamespace(name), new DebugSubscription(null));
    }
    ...
    private static <T> DebugSubscription<T> registerWithValue(String name, StreamCodec<? super RegistryFriendlyByteBuf, T> streamCodec) {
        return Registry.register(BuiltInRegistries.DEBUG_SUBSCRIPTION, ResourceLocation.withDefaultNamespace(name), new DebugSubscription<>(streamCodec));
    }
    ...
    private static <T> DebugSubscription<T> registerTemporaryValue(String name, StreamCodec<? super RegistryFriendlyByteBuf, T> streamCodec, int expireAfterTicks) {
        return Registry.register(
            BuiltInRegistries.DEBUG_SUBSCRIPTION, ResourceLocation.withDefaultNamespace(name), new DebugSubscription<>(streamCodec, expireAfterTicks)
        );
    }
}
