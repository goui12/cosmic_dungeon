...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
    ...
    private static final Comparator<Holder.Reference<?>> HOLDER_KEY_COMPARATOR = Comparator.comparing(p_335187_ -> p_335187_.key().location());
    ...
    private static Advancement.Builder addLeashedFrogVariants(
        ...
        sortedVariants(frogVariants)
            .forEach(
                p_411024_ -> builder.addCriterion(
                    p_411024_.key().location().toString(),
                    PlayerInteractTrigger.TriggerInstance.itemUsedOnEntity(
                        ...
                        Optional.of(
                            EntityPredicate.wrap(
                                EntityPredicate.Builder.entity()
                                    ...
                                    .components(
                                        DataComponentMatchers.Builder.components()
                                            .exact(DataComponentExactPredicate.expect(DataComponents.FROG_VARIANT, p_411024_))
                                            .build()
                                    )
                            )
                        )
    ...
    private static Advancement.Builder addCatVariants(Advancement.Builder builder, HolderLookup<CatVariant> variantRegistry) {
        sortedVariants(variantRegistry)
            .forEach(
                p_411018_ -> builder.addCriterion(
                    p_411018_.key().location().toString(),
                    TameAnimalTrigger.TriggerInstance.tamedAnimal(
                        EntityPredicate.Builder.entity()
                            .components(
                                DataComponentMatchers.Builder.components()
                                    .exact(DataComponentExactPredicate.expect(DataComponents.CAT_VARIANT, p_411018_))
                                    .build()
                            )
                    )
                )
    ...
    private static Advancement.Builder addTamedWolfVariants(Advancement.Builder builder, HolderLookup<WolfVariant> variantRegistry) {
        sortedVariants(variantRegistry)
            .forEach(
                p_411020_ -> builder.addCriterion(
                    p_411020_.key().location().toString(),
                    TameAnimalTrigger.TriggerInstance.tamedAnimal(
                        EntityPredicate.Builder.entity()
                            .components(
                                DataComponentMatchers.Builder.components()
                                    .exact(DataComponentExactPredicate.expect(DataComponents.WOLF_VARIANT, p_411020_))
                                    .build()
                            )
                    )
                )
