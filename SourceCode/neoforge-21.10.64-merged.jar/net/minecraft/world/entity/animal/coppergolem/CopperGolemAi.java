...
public class CopperGolemAi {
    ...
    private static final Predicate<BlockState> TRANSPORT_ITEM_SOURCE_BLOCK = p_435104_ -> p_435104_.is(BlockTags.COPPER_CHESTS);
    private static final Predicate<BlockState> TRANSPORT_ITEM_DESTINATION_BLOCK = p_432974_ -> p_432974_.is(Blocks.CHEST) || p_432974_.is(Blocks.TRAPPED_CHEST);
    ...
    private static TransportItemsBetweenContainers.OnTargetReachedInteraction onReachedTargetInteraction(
        CopperGolemState statue, @Nullable SoundEvent sound
    ) {
        return (p_442464_, p_442465_, p_442466_) -> {
            if (p_442464_ instanceof CopperGolem coppergolem) {
                Container container = p_442465_.container();
                if (p_442466_ == 1) {
                    container.startOpen(coppergolem);
                    coppergolem.setOpenedChestPos(p_442465_.pos());
                    coppergolem.setState(statue);
                }

                if (p_442466_ == 9 && sound != null) {
                    coppergolem.playSound(sound);
                }

                if (p_442466_ == 60) {
                    if (container.getEntitiesWithContainerOpen().contains(p_442464_)) {
    ...
    private static Consumer<PathfinderMob> onTravelling() {
        return p_434110_ -> {
            if (p_434110_ instanceof CopperGolem coppergolem) {
                coppergolem.clearOpenedChestPos();
    ...
    private static Predicate<TransportItemsBetweenContainers.TransportItemTarget> shouldQueueForTarget() {
        return p_435263_ -> p_435263_.blockEntity() instanceof ChestBlockEntity chestblockentity
