    ...
    public static BehaviorControl<PathfinderMob> create(float speedModifier) {
        ...
        return BehaviorBuilder.create(
            p_258633_ -> p_258633_.group(p_258633_.absent(MemoryModuleType.WALK_TARGET), p_258633_.absent(MemoryModuleType.HOME))
                .apply(
                    p_258633_,
                    (p_258626_, p_258627_) -> (p_448944_, p_448945_, p_448946_) -> {
                        if (p_448944_.getGameTime() - mutablelong.getValue() < 20L) {
                            return false;
                        } else {
                            PoiManager poimanager = p_448944_.getPoiManager();
                            Optional<BlockPos> optional = poimanager.findClosest(
                                p_217376_ -> p_217376_.is(PoiTypes.HOME), p_448945_.blockPosition(), 48, PoiManager.Occupancy.ANY
                            );
                            if (!optional.isEmpty() && !(optional.get().distSqr(p_448945_.blockPosition()) <= 4.0)) {
                                MutableInt mutableint = new MutableInt(0);
                                mutablelong.setValue(p_448944_.getGameTime() + p_448944_.getRandom().nextInt(20));
                                ...
                                Set<Pair<Holder<PoiType>, BlockPos>> set = poimanager.findAllWithType(
                                        p_217372_ -> p_217372_.is(PoiTypes.HOME), predicate, p_448945_.blockPosition(), 48, PoiManager.Occupancy.ANY
                                    )
                                    .collect(Collectors.toSet());
                                Path path = AcquirePoi.findPathToPois(p_448945_, set);
                                if (path != null && path.canReach()) {
                                    ...
                                    if (optional1.isPresent()) {
                                        ...
                                        p_448944_.debugSynchronizers().updatePoi(blockpos);
                                    }
