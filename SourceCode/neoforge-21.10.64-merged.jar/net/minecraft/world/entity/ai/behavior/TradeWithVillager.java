...
public class TradeWithVillager extends Behavior<Villager> {
    ...
    protected void tick(ServerLevel level, Villager owner, long gameTime) {
        ...
        if (!(owner.distanceToSqr(villager) > 5.0)) {
            ...
            boolean flag = owner.getVillagerData().profession().is(VillagerProfession.FARMER);
            if (owner.hasExcessFood() && (flag || villager.wantsMoreFood())) {
                throwHalfStack(owner, Villager.FOOD_POINTS.keySet(), villager);
            }

            if (flag && owner.getInventory().countItem(Items.WHEAT) > Items.WHEAT.getDefaultMaxStackSize() / 2) {
    ...
    private static Set<Item> figureOutWhatIAmWillingToTrade(Villager villager, Villager other) {
        ImmutableSet<Item> immutableset = other.getVillagerData().profession().value().requestedItems();
