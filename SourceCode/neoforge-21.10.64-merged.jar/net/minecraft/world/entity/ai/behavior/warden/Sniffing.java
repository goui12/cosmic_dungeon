...
public class Sniffing<E extends Warden> extends Behavior<E> {
    ...
    protected void stop(ServerLevel level, E entity, long gameTime) {
        ...
        entity.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(entity::canTargetEntity).ifPresent(p_427042_ -> {
            if (entity.closerThan(p_427042_, 6.0, 20.0)) {
                entity.increaseAngerAt(p_427042_);
            }

            if (!entity.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
                WardenAi.setDisturbanceLocation(entity, p_427042_.blockPosition());
            }
        });
    }
}
