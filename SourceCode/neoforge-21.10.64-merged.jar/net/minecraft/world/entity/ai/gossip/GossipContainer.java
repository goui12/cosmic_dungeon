...
public class GossipContainer {
    public static final Codec<GossipContainer> CODEC = GossipContainer.GossipEntry.CODEC
        .listOf()
        .xmap(GossipContainer::new, p_404372_ -> p_404372_.unpack().toList());
    ...
    private GossipContainer(List<GossipContainer.GossipEntry> entries) {
        entries.forEach(p_26162_ -> this.getOrCreate(p_26162_.target).entries.put(p_26162_.type, p_26162_.value));
    }
    ...
    public long getCountForType(GossipType gossipType, DoublePredicate gossipPredicate) {
        return this.gossips.values().stream().filter(p_148174_ -> gossipPredicate.test(p_148174_.entries.getOrDefault(gossipType, 0) * gossipType.weight)).count();
    }
    ...
    public void putAll(GossipContainer other) {
        other.gossips.forEach((p_404373_, p_404374_) -> this.getOrCreate(p_404373_).entries.putAll(p_404374_.entries));
    }
