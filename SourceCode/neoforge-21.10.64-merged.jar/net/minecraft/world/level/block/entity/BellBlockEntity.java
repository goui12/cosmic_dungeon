...
public class BellBlockEntity extends BlockEntity {
    ...
    private static void showBellParticles(Level level, BlockPos pos, List<LivingEntity> raiders) {
        ...
        int i = (int)raiders.stream().filter(p_427191_ -> pos.closerToCenterThan(p_427191_.position(), 48.0)).count();
        raiders.stream()
            ...
            .forEach(
                p_333683_ -> {
                    float f = 1.0F;
                    double d0 = Math.sqrt(
                        (p_333683_.getX() - pos.getX()) * (p_333683_.getX() - pos.getX())
                            + (p_333683_.getZ() - pos.getZ()) * (p_333683_.getZ() - pos.getZ())
                    );
                    double d1 = pos.getX() + 0.5F + 1.0 / d0 * (p_333683_.getX() - pos.getX());
                    double d2 = pos.getZ() + 0.5F + 1.0 / d0 * (p_333683_.getZ() - pos.getZ());
                    int j = Mth.clamp((i - 21) / -2, 3, 15);

                    for (int k = 0; k < j; k++) {
                        int l = mutableint.addAndGet(5);
                        level.addParticle(ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, l), d1, pos.getY() + 0.5F, d2, 0.0, 0.0, 0.0);
                    }
                }
            );
    }
