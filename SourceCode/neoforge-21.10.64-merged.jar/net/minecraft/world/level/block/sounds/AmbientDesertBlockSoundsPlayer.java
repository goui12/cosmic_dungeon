...
public class AmbientDesertBlockSoundsPlayer {
        ...
        if (random.nextInt(130) == 0) {
            ...
            if (shouldPlayDesertDryVegetationBlockSounds(level, pos.below())) {
                level.playLocalSound(
                    pos.getX(), pos.getY(), pos.getZ(), SoundEvents.DEAD_BUSH_IDLE, SoundSource.AMBIENT, 1.0F, 1.0F, false
                );
            }
        }
    }
    ...
    private static boolean shouldPlayAmbientSandSound(Level level, BlockPos pos) {
        ...
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            blockpos$mutableblockpos.set(pos).move(direction, 8);
            if (columnContainsTriggeringBlock(level, blockpos$mutableblockpos) && i++ >= 3) {
                return true;
            }
    ...
    private static boolean columnContainsTriggeringBlock(Level level, BlockPos.MutableBlockPos pos) {
        ...
        if (Math.abs(i - pos.getY()) > 5) {
            ...
            boolean flag = level.getBlockState(pos.setY(i + 1)).isAir();
            return flag && canTriggerAmbientDesertSandSounds(level.getBlockState(pos.setY(i)));
        }
    }
