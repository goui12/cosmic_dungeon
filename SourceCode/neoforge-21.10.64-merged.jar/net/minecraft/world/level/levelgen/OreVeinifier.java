...
public final class OreVeinifier {
    ...
    protected static NoiseChunk.BlockStateFiller create(
        ...
        return p_448993_ -> {
            double d0 = veinToggle.compute(p_448993_);
            int i = p_448993_.blockY();
            ...
            if (k >= 0 && j >= 0) {
                ...
                if (d1 + d2 < 0.4F) {
                    ...
                    RandomSource randomsource = random.at(p_448993_.blockX(), i, p_448993_.blockZ());
                    if (randomsource.nextFloat() > 0.7F) {
                        return blockstate;
                    } else if (veinRidged.compute(p_448993_) >= 0.0) {
                        return blockstate;
                    } else {
                        double d3 = Mth.clampedMap(d1, 0.4F, 0.6F, 0.1F, 0.3F);
                        if (randomsource.nextFloat() < d3 && veinGap.compute(p_448993_) > -0.3F) {
