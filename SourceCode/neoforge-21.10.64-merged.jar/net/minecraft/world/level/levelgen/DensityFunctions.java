...
public final class DensityFunctions {
        ...
        public double compute(DensityFunction.FunctionContext context) {
            ...
            if (i <= this.lowerBound) {
                ...
                for (int j = i; j >= this.lowerBound; j -= this.cellHeight) {
                    if (this.density.compute(new DensityFunction.SinglePointContext(context.blockX(), j, context.blockZ())) > 0.0) {
                        return j;
                    }
                }
    ...
    protected record ShiftedNoise(
        ...
        @Override
        public double compute(DensityFunction.FunctionContext context) {
            double d0 = context.blockX() * this.xzScale + this.shiftX.compute(context);
    ...
    public record Spline(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> spline) implements DensityFunction {
        ...
        public record Coordinate(Holder<DensityFunction> function) implements BoundedFloatFunction<DensityFunctions.Spline.Point> {
            ...
            public float apply(DensityFunctions.Spline.Point input) {
                return (float)this.function.value().compute(input.context());
            }
    ...
    protected record WeirdScaledSampler(
        ...
        public double transform(DensityFunction.FunctionContext context, double value) {
            ...
            return d0 * Math.abs(this.noise.getValue(context.blockX() / d0, context.blockY() / d0, context.blockZ() / d0));
        }
