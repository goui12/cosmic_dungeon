...
public final class BelowZeroRetrogen {
    ...
    public static void replaceOldBedrock(ProtoChunk chunk) {
        int i = 4;
        BlockPos.betweenClosed(0, 0, 0, 15, 4, 15).forEach(p_393429_ -> {
            if (chunk.getBlockState(p_393429_).is(Blocks.BEDROCK)) {
                chunk.setBlockState(p_393429_, Blocks.DEEPSLATE.defaultBlockState());
            }
        });
    }
        ...
        for (int k = 0; k < 16; k++) {
            for (int l = 0; l < 16; l++) {
                if (this.hasBedrockHole(k, l)) {
                    BlockPos.betweenClosed(k, i, l, k, j, l).forEach(p_393431_ -> chunk.setBlockState(p_393431_, Blocks.AIR.defaultBlockState()));
                }
            }
        }
    }
