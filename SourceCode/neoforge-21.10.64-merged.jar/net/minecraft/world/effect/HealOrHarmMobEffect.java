...
class HealOrHarmMobEffect extends InstantenousMobEffect {
    ...
    public void applyInstantenousEffect(
        ...
        if (this.isHarm == entity.isInvertedHealAndHarm()) {
            ...
            int j = (int)(health * (6 << amplifier) + 0.5);
            if (source == null) {
                entity.hurtServer(level, entity.damageSources().magic(), j);
            } else {
                entity.hurtServer(level, entity.damageSources().indirectMagic(source, indirectSource), j);
            }
        }
    }
}
