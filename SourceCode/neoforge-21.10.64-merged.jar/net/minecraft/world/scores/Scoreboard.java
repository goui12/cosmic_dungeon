            ...
            .flatMap(
                p_400991_ -> {
                    ...
                    return p_400991_.getValue()
                        ...
                        .entrySet()
                        .stream()
                        .map(p_400993_ -> new Scoreboard.PackedScore(s, p_400993_.getKey().getName(), p_400993_.getValue()));
                }
            )
            .toList();
    }
    ...
    public record PackedScore(String owner, String objective, Score score) {
        public static final Codec<Scoreboard.PackedScore> CODEC = RecordCodecBuilder.create(
            p_401229_ -> p_401229_.group(
                ...
                .apply(p_401229_, Scoreboard.PackedScore::new)
        );
    }
}
