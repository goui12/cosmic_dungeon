...
public class DebugScreenEntries {
    ...
    private static ResourceLocation register(String name, DebugScreenEntry entry) {
        return register(ResourceLocation.withDefaultNamespace(name), entry);
    }

    private static ResourceLocation register(ResourceLocation name, DebugScreenEntry entry) {
        ENTRIES_BY_LOCATION.put(name, entry);
        return name;
    }
    ...
    @Nullable
    public static DebugScreenEntry getEntry(ResourceLocation name) {
        return ENTRIES_BY_LOCATION.get(name);
    }
    ...
    static {
        ...
        PROFILES = net.neoforged.fml.ModLoader.postEventWithReturn(new net.neoforged.neoforge.client.event.RegisterDebugEntriesEvent(ENTRIES_BY_LOCATION, map, map1)).validateProfiles();
    }
}
