...
public class ChatComponent {
    ...
    private int forEachLine(int linesPerPage, int tickCount, boolean focused, int bottomY, ChatComponent.LineConsumer action) {
        int i = this.getLineHeight();
        int j = 0;

        for (int k = Math.min(this.trimmedMessages.size() - this.chatScrollbarPos, linesPerPage) - 1; k >= 0; k--) {
            int l = k + this.chatScrollbarPos;
            GuiMessage.Line guimessage$line = this.trimmedMessages.get(l);
            if (guimessage$line != null) {
                int i1 = tickCount - guimessage$line.addedTime();
                float f = focused ? 1.0F : (float)getTimeFactor(i1);
                if (f > 1.0E-5F) {
                    j++;
                    int j1 = bottomY - k * i;
                    int k1 = j1 - i;
                    action.accept(0, k1, j1, guimessage$line, k, f);
                }
            }
        }
    ...
    public void render(GuiGraphics guiGraphics, int tickCount, int mouseX, int mouseY, boolean focused) {
        if (!this.isChatHidden()) {
            ...
            if (j > 0) {
                ...
                this.forEachLine(i, tickCount, focused, i1, (p_426861_, p_426862_, p_426863_, p_426864_, p_426865_, p_426866_) -> {
                    guiGraphics.fill(p_426861_ - 4, p_426862_, p_426861_ + k + 4 + 4, p_426863_, ARGB.color(p_426866_ * f2, -16777216));
                    GuiMessageTag guimessagetag = p_426864_.tag();
                    if (guimessagetag != null) {
                        int l4 = ARGB.color(p_426866_ * f1, guimessagetag.indicatorColor());
                        guiGraphics.fill(p_426861_ - 4, p_426862_, p_426861_ - 2, p_426863_, l4);
                        if (p_426865_ == j1 && guimessagetag.icon() != null) {
                            int i5 = this.getTagIconLeft(p_426864_);
                            int j5 = p_426863_ + k1 + 9;
                            this.drawTagIcon(guiGraphics, i5, j5, guimessagetag.icon());
                        }
                    }
                });
                int l1 = this.forEachLine(i, tickCount, focused, i1, (p_426870_, p_426871_, p_426872_, p_426873_, p_426874_, p_426875_) -> {
                    int l4 = p_426872_ + k1;
                    guiGraphics.drawString(this.minecraft.font, p_426873_.content(), p_426870_, l4, ARGB.color(p_426875_ * f1, -1));
                });
    ...
    public boolean handleChatQueueClicked(double mouseX, double mouseY) {
        if (this.isChatFocused() && !this.minecraft.options.hideGui && !this.isChatHidden()) {
            ...
            if (chatlistener.queueSize() == 0L) {
                ...
                double d1 = this.minecraft.getWindow().getGuiScaledHeight() - mouseY - 40.0;
    ...
    private double screenToChatY(double y) {
        double d0 = this.minecraft.getWindow().getGuiScaledHeight() - y - 40.0;
    ...
    interface LineConsumer {
        void accept(int x, int startY, int endY, GuiMessage.Line line, int index, float fade);
    }
