...
public class FoxModel extends EntityModel<FoxRenderState> {
    ...
    public void setupAnim(FoxRenderState renderState) {
        ...
        this.head.zRot = renderState.headRollAngle;
        ...
        if (renderState.isCrouching) {
            ...
            float f3 = renderState.crouchAmount;
            this.body.y += f3 * f2;
            this.head.y += f3 * f2;
        } else if (renderState.isSleeping) {
        ...
        if (!renderState.isSleeping && !renderState.isFaceplanted && !renderState.isCrouching) {
        ...
        if (renderState.isSleeping) {
            this.head.xRot = 0.0F;
            this.head.yRot = (float) (-Math.PI * 2.0 / 3.0);
            this.head.zRot = Mth.cos(renderState.ageInTicks * 0.027F) / 22.0F;
        }

        if (renderState.isCrouching) {
            float f4 = Mth.cos(renderState.ageInTicks) * 0.01F;
            this.body.yRot = f4;
