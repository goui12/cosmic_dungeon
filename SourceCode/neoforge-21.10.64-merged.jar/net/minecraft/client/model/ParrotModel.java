...
public class ParrotModel extends EntityModel<ParrotRenderState> {
    ...
    public void setupAnim(ParrotRenderState renderState) {
        super.setupAnim(renderState);
        this.prepare(renderState.pose);
        ...
        switch (renderState.pose) {
            case STANDING:
                this.leftLeg.xRot = this.leftLeg.xRot + Mth.cos(renderState.walkAnimationPos * 0.6662F) * 1.4F * renderState.walkAnimationSpeed;
                this.rightLeg.xRot = this.rightLeg.xRot + Mth.cos(renderState.walkAnimationPos * 0.6662F + (float) Math.PI) * 1.4F * renderState.walkAnimationSpeed;
            case FLYING:
            case ON_SHOULDER:
            default:
                float f2 = renderState.flapAngle * 0.3F;
                this.head.y += f2;
                this.tail.xRot = this.tail.xRot + Mth.cos(renderState.walkAnimationPos * 0.6662F) * 0.3F * renderState.walkAnimationSpeed;
                this.tail.y += f2;
                this.body.y += f2;
                this.leftWing.zRot = -0.0873F - renderState.flapAngle;
                this.leftWing.y += f2;
                this.rightWing.zRot = 0.0873F + renderState.flapAngle;
                this.rightWing.y += f2;
                this.leftLeg.y += f2;
                this.rightLeg.y += f2;
            case SITTING:
                break;
            case PARTY:
                float f = Mth.cos(renderState.ageInTicks);
                float f1 = Mth.sin(renderState.ageInTicks);
                this.head.x += f;
                this.head.y += f1;
                this.head.xRot = 0.0F;
                this.head.yRot = 0.0F;
                this.head.zRot = Mth.sin(renderState.ageInTicks) * 0.4F;
                this.body.x += f;
                this.body.y += f1;
                this.leftWing.zRot = -0.0873F - renderState.flapAngle;
                this.leftWing.x += f;
