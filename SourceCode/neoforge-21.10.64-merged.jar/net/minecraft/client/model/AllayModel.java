...
public class AllayModel extends EntityModel<AllayRenderState> implements ArmedModel<AllayRenderState> {
    ...
    public void setupAnim(AllayRenderState renderState) {
        super.setupAnim(renderState);
        float f = renderState.walkAnimationSpeed;
        float f1 = renderState.walkAnimationPos;
        float f2 = renderState.ageInTicks * 20.0F * (float) (Math.PI / 180.0) + f1;
        float f3 = Mth.cos(f2) * (float) Math.PI * 0.15F + f;
        float f4 = renderState.ageInTicks * 9.0F * (float) (Math.PI / 180.0);
        float f5 = Math.min(f / 0.3F, 1.0F);
        float f6 = 1.0F - f5;
        float f7 = renderState.holdingAnimationProgress;
        if (renderState.isDancing) {
            float f8 = renderState.ageInTicks * 8.0F * (float) (Math.PI / 180.0) + f;
            float f9 = Mth.cos(f8) * 16.0F * (float) (Math.PI / 180.0);
            float f10 = renderState.spinningProgress;
            float f11 = Mth.cos(f8) * 14.0F * (float) (Math.PI / 180.0);
            float f12 = Mth.cos(f8) * 30.0F * (float) (Math.PI / 180.0);
            this.root.yRot = renderState.isSpinning ? (float) (Math.PI * 4) * f10 : this.root.yRot;
    ...
    public void translateToHand(AllayRenderState renderState, HumanoidArm arm, PoseStack poseStack) {
        float f = 1.0F;
        float f1 = 3.0F;
