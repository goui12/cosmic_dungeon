...
public class ArmadilloModel extends EntityModel<ArmadilloRenderState> {
    ...
    public void setupAnim(ArmadilloRenderState renderState) {
        super.setupAnim(renderState);
        if (renderState.isHidingInShell) {
            ...
            this.head.xRot = Mth.clamp(renderState.xRot, -22.5F, 25.0F) * (float) (Math.PI / 180.0);
            this.head.yRot = Mth.clamp(renderState.yRot, -32.5F, 32.5F) * (float) (Math.PI / 180.0);
        }

        this.walkAnimation.applyWalk(renderState.walkAnimationPos, renderState.walkAnimationSpeed, 16.5F, 2.5F);
        this.rollOutAnimation.apply(renderState.rollOutAnimationState, renderState.ageInTicks);
        this.rollUpAnimation.apply(renderState.rollUpAnimationState, renderState.ageInTicks);
        this.peekAnimation.apply(renderState.peekAnimationState, renderState.ageInTicks);
    }
}
