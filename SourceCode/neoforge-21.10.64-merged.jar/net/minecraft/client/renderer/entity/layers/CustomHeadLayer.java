...
public class CustomHeadLayer<S extends LivingEntityRenderState, M extends EntityModel<S> & HeadedModel> extends RenderLayer<S, M> {
    ...
    public CustomHeadLayer(RenderLayerParent<S, M> renderer, EntityModelSet modelSet, PlayerSkinRenderCache playerSkinRenderCache, CustomHeadLayer.Transforms transforms) {
        ...
        this.skullModels = Util.memoize(p_386732_ -> SkullBlockRenderer.createModel(modelSet, p_386732_));
        this.playerSkinRenderCache = playerSkinRenderCache;
    }

    public void submit(PoseStack poseStack, SubmitNodeCollector nodeCollector, int packedLight, S renderState, float yRot, float xRot) {
        if (!renderState.headItem.isEmpty() || renderState.wornHeadType != null) {
            poseStack.pushPose();
            ...
            if (renderState.wornHeadType != null) {
                poseStack.translate(0.0F, this.transforms.skullYOffset(), 0.0F);
                poseStack.scale(1.1875F, -1.1875F, -1.1875F);
                poseStack.translate(-0.5, 0.0, -0.5);
                SkullBlock.Type skullblock$type = renderState.wornHeadType;
                SkullModelBase skullmodelbase = this.skullModels.apply(skullblock$type);
                RenderType rendertype = this.resolveSkullRenderType(renderState, skullblock$type);
                SkullBlockRenderer.submitSkull(
                    null, 180.0F, renderState.wornHeadAnimationPos, poseStack, nodeCollector, packedLight, skullmodelbase, rendertype, renderState.outlineColor, null
                );
            } else {
                translateToHead(poseStack, this.transforms);
                renderState.headItem.submit(poseStack, nodeCollector, packedLight, OverlayTexture.NO_OVERLAY, renderState.outlineColor);
            }
    ...
    private RenderType resolveSkullRenderType(LivingEntityRenderState renderState, SkullBlock.Type type) {
