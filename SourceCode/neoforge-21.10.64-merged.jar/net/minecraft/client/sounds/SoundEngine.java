...
public class SoundEngine {
    ...
    public void updateCategoryVolume(SoundSource category) {
        if (this.loaded) {
            if ((category == SoundSource.MASTER || category == SoundSource.MUSIC) && this.options.getFinalSoundSourceVolume(SoundSource.MUSIC) > 0.0F) {
            ...
            this.instanceToChannel.forEach((p_120280_, p_120281_) -> {
                ...
                p_120281_.execute(p_425183_ -> p_425183_.setVolume(f));
            });
        }
    }
    ...
    public void setVolume(SoundInstance soundInstance, float volume) {
        if (this.loaded) {
            ...
            if (channelaccess$channelhandle != null) {
                channelaccess$channelhandle.execute(p_382558_ -> p_382558_.setVolume(volume * this.calculateVolume(soundInstance)));
            }
        }
    }
    ...
    private float calculateVolume(float volumeMultiplier, SoundSource source) {
        return Mth.clamp(volumeMultiplier, 0.0F, 1.0F) * Mth.clamp(this.options.getFinalSoundSourceVolume(source), 0.0F, 1.0F);
    }
